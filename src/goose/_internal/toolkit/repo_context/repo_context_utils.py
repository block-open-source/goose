...
def get_repo_size(repo_path: str) -> int:
    ...
    return _get_directory_size(git_dir) / (1024**2)


def _get_files_and_directories(root_dir: str) -> Dict[str, list]:
...
def goose_picks_files(root: str, exchange: Exchange, max_workers: int = 4) -> List[str]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        while queue:
            ...
            futures = {executor.submit(_process_directory, dir, exchange): dir for dir in current_batch}
...
def _process_directory(current_dir: str, exchange: Exchange) -> Tuple[List[str], List[str]]:
    ...
    """
    files_and_dirs = _get_files_and_directories(current_dir)
